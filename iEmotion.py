...
face_cascade = cv2.CascadeClassifier("face_detectors/haarcascade_frontalface_default.xml")
...
if args["save"]:
    out = cv2.VideoWriter("outputs/recordings/" + "liveoutput.avi",
...
while True:
    ...
    for coords in face_coord:
        ...
        img_cp = cv2.resize(img_cp, (48, 48))
        img_cp = img_cp.reshape([-1, 48, 48, 1])
        img_cp = np.multiply(img_cp, 1.0 / 255.0)

        predicted_proba = model.predict(img_cp)
        predicted_label = int(np.argmax(predicted_proba[0]))
        ...
        cv2.putText(frame, str(
            emotion_labels[predicted_label]), (10, frame.shape[0] - 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (225, 255, 255), 2)
        ...
        time_value = data_time(time_value, emotion_labels[predicted_label], predicted_proba, ear)
...
if args["savedata"]:
    ...
    df.to_csv('outputs/recordings'+'/exportlive.csv')
